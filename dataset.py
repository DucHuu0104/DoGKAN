...
from utils import pad_T_global, extract_feature_from_deLDS
...
def get_dann_loaders(X, y, persons, target_person, batch_src=32, batch_tgt=32, val_size=0.2, random_state=12):
    ...
    src_train_loader = DataLoader(EEGDataset(X_train, y_train), batch_size=batch_src, shuffle=True, drop_last=True)
    src_val_loader = DataLoader(EEGDataset(X_val, y_val), batch_size=batch_src, shuffle=False)
    tgt_loader = DataLoader(EEGDataset(X_tgt, y_tgt), batch_size=batch_tgt, shuffle=True, drop_last=True) # discriminator needs shuffle target
